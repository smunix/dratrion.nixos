        ...
        function(widget, stdout)
          ...
          for k,v in ipairs(mounts) do
            ...
            local row = wibox.widget{
              ...
              {
                text = (function (disks, v)
                    if math.floor(disks[v].size) < 1024 then
                      -- B
                      return math.floor(disks[v].used)
                        .. '/'
                        .. math.floor(disks[v].size) .. 'KiB('
                        .. math.floor(disks[v].perc) .. '%)'
                    elseif math.floor(disks[v].size) < 1024*1024 then
                      -- KiB
                      return math.floor(math.floor(disks[v].used)/1024)
                        .. '/'
                        .. math.floor(math.floor(disks[v].size)/1024) .. 'MiB('
                        .. math.floor(disks[v].perc) .. '%)'
                    elseif math.floor(disks[v].size) < 1024*1024*1024 then
                      -- MiB
                      return math.floor(math.floor(disks[v].used)/1024/1024)
                        .. '/'
                        .. math.floor(math.floor(disks[v].size)/1024/1024) .. 'GiB('
                        .. math.floor(disks[v].perc) .. '%)'
                    elseif math.floor(disks[v].size) < 1024*1024*1024*1024 then
                      -- GiB
                      return math.floor(math.floor(disks[v].used)/1024/1024/1024)
                        .. '/'
                        .. math.floor(math.floor(disks[v].size)/1024/1024/1024) .. 'PiB('
                        .. math.floor(disks[v].perc) .. '%)'
                    end
                    return math.floor(disks[v].used)
                      .. '/'
                      .. math.floor(disks[v].size) .. 'B('
                      .. math.floor(disks[v].perc) .. '%)'
                       end)(disks, v)
                  ,
                  widget = wibox.widget.textbox
